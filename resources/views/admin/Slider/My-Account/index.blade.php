                    ...
                    <div class="d-flex m-b-10 no-block">
                        ...
                        <div class="ml-auto text-light-blue">
                            <ul class="nav nav-tabs customtab default-customtab list-inline text-uppercase lp-5 font-medium font-12" role="tablist">
                                <li>
                                    <a href="{{url('panel/admin/my-account-slider/create/')}}" class="btn waves-effect waves-light btn-rounded btn-primary">Add New Slider</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    @if($my_account_sliders)
                        <div class="table-responsive m-t-10">
                            <table id="myTable" class="table color-table table-bordered product-table table-hover">
                                ...
                                <tbody>
                                @foreach($my_account_sliders as $key => $slider)
                                    <tr>
                                        <td>{{ $key +1 }}</td>
                                         <td><img src="{{ asset($slider->image_path) }}" class="img-responsive"></td>
                                        <td>{!! $slider->page_name !!}</td>
                                        <td class="text-center">
                                            <a href="{{ url('/panel/admin/my-account-slider/'.$slider->id.'/edit/') }}"><i class="fas fa-edit"></i></a>
                                            <form style="display: inline-block;" method="POST" action="{{ url('/panel/admin/slider',$slider->id) }}">
                                            @csrf
                                            @method('DELETE')
